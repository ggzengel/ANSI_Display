...
void draw_orientation_and_borders(void)
{
    ...
    // Outer border
    for (uint16_t x = 0; x <= LCD_1IN47.WIDTH - 1; ++x)
    {
        LCD_1IN47_DisplayPoint(x, 0, BLACK);
        LCD_1IN47_DisplayPoint(x, LCD_1IN47.HEIGHT - 1, BLACK);
    }
    for (uint16_t y = 0; y <= LCD_1IN47.HEIGHT - 1; ++y)
    {
        LCD_1IN47_DisplayPoint(0, y, BLACK);
        LCD_1IN47_DisplayPoint(LCD_1IN47.WIDTH - 1, y, BLACK);
    }
    // Inner border (10px inset)
    for (uint16_t x = 10; x <= LCD_1IN47.WIDTH - 11; ++x)
    {
        LCD_1IN47_DisplayPoint(x, 10, GRAY);
        LCD_1IN47_DisplayPoint(x, LCD_1IN47.HEIGHT - 11, GRAY);
    }
    for (uint16_t y = 10; y <= LCD_1IN47.HEIGHT - 11; ++y)
    {
        LCD_1IN47_DisplayPoint(10, y, GRAY);
        LCD_1IN47_DisplayPoint(LCD_1IN47.WIDTH - 11, y, GRAY);
    }

    // Draw a large 'T' in the center using two lines
    uint16_t cx = LCD_1IN47.WIDTH / 2;
    uint16_t cy = LCD_1IN47.HEIGHT / 2;
    uint16_t T_width = LCD_1IN47.WIDTH / 4;
    uint16_t T_height = LCD_1IN47.HEIGHT / 3;

    // Horizontal bar of T
    for (uint16_t x = cx - T_width / 2; x <= cx + T_width / 2 && x <= LCD_1IN47.WIDTH - 1; ++x)
    {
        if (x < LCD_1IN47.WIDTH)
        {
    ...
    // Vertical bar of T
    for (uint16_t y = cy - T_height / 2; y <= cy + T_height / 2 && y <= LCD_1IN47.HEIGHT - 1; ++y)
    {
        if (y < LCD_1IN47.HEIGHT)
        {
...
int main(void)
{
    ...
    const char *serial_msg = "\x1b[0mSerial: ";
    for (const char *p = serial_msg; *p; ++p)
    {
        process_char(*p);
    }
